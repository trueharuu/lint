...
export function dbg<T>(x: T): T {
  if (typeof x === 'object' && !Array.isArray(x)) {
    ...
    if ('parent' in (x as object)) {
      // @ts-expect-error yes it does
      delete x.parent;
    }
  }

  let t = util.formatWithOptions(
    {
      depth: null,
...
export function color(s: any, code: any, reset: any = 0): string {
  return `\x1b[${code}m${s}\x1b[0m\x1b[${reset}m`;
}
...
export function print_node(
  ...
  dbg(
    `${'  '.repeat(d)}${color(i, 30)} ${formatSyntaxKind(node.kind)}[${color(
      ...
      node.getText(ast).trim().includes('\n')
        ? '...'
...
export function map_with_spans(
  ...
  cb: (text: string, span: [number, number]) => string,
): string {
  return indices
    .reduce((str, [start, end, ..._]) => {
      str[start] = cb(str.slice(start, end).join(''), [start, end, ..._]);
...
export function print(t: Node, sourceFile: SourceFile) {
